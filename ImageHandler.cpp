...
ImageHandler::ImageHandler(char *pcFilePath)
    : uiWidth(0)
    , uiHeight(0)
    , uiMaxValue(0)
    , pfImageData(NULL)
...
int ImageHandler::removeBlanks(FILE *pFile)
{
    int iBlankcount = 0;
    int iTmpChar;
    int iIsContinue = 1;
...
int ImageHandler::extractJpgHead(FILE *pFile)
{
    char acFileType[2];
    bool  bIsRawType;
...
unsigned int ImageHandler::getPixelSize(void)
{
    return (this->uiMaxValue >= 256) + 1;
}

/* get image size (pixel number) */
unsigned int ImageHandler::getImageSize(void)
{
    return this->uiHeight * this->uiWidth;
}

/* extract image data */
int ImageHandler::extractPJpgData(FILE* pFile)
{
    size_t  ulReadSize = 0;
    int     iRet = 0;
...
/* get image data */
float *ImageHandler::getImageData(void)
{
    return this->pfImageData;
}

/* get image height */
unsigned int ImageHandler::getHeight(void)
{
    return this->uiHeight;
}

/* get image height */
unsigned int ImageHandler::getWidth(void)
{
    return this->uiWidth;
}

/* create image based on path */
int ImageHandler::createImage(void)
{
    FILE *pFile = NULL;
    int   iRet = 0;
    unsigned int uiPixelSize;
